...
<div class="bg-white border border-slate-200 rounded-2xl p-6 sm:p-8 shadow-sm max-w-3xl">
    <h2 class="text-xl font-heading font-bold text-brand mb-6">Edit Dokumen DIP</h2>
    ...
    <form action="{{ route('admin.dip-documents.update', $dipDocument->id) }}" method="POST" enctype="multipart/form-data" class="space-y-5 text-sm">
        @csrf
        @method('PUT')
        ...
        <div class="grid grid-cols-1 md:grid-cols-2 gap-5">
            <div>
                ...
                <input type="text" name="registration_number" value="{{ old('registration_number', $dipDocument->registration_number) }}" required class="w-full bg-slate-50 border border-slate-200 rounded-xl px-3.5 py-2.5 text-slate-800 font-mono focus:outline-none focus:border-teal-500">
            </div>
            <div>
                <label class="block text-slate-700 mb-1.5 font-semibold">Tahun *</label>
                <input type="number" name="year" value="{{ old('year', $dipDocument->year) }}" required class="w-full bg-slate-50 border border-slate-200 rounded-xl px-3.5 py-2.5 text-slate-800 focus:outline-none focus:border-teal-500">
            </div>
        </div>

        <div>
            <label class="block text-slate-700 mb-1.5 font-semibold">Judul Dokumen *</label>
            <input type="text" name="title" value="{{ old('title', $dipDocument->title) }}" required class="w-full bg-slate-50 border border-slate-200 rounded-xl px-3.5 py-2.5 text-slate-800 focus:outline-none focus:border-teal-500">
        </div>
        ...
        <div>
            ...
            <select name="category" required class="w-full bg-slate-50 border border-slate-200 rounded-xl px-3.5 py-2.5 text-slate-800 focus:outline-none focus:border-teal-500">
                <option value="berkala" {{ old('category', $dipDocument->category) == 'berkala' ? 'selected' : '' }}>Informasi Berkala</option>
                <option value="serta-merta" {{ old('category', $dipDocument->category) == 'serta-merta' ? 'selected' : '' }}>Informasi Serta Merta</option>
                <option value="setiap-saat" {{ old('category', $dipDocument->category) == 'setiap-saat' ? 'selected' : '' }}>Informasi Setiap Saat</option>
                <option value="dikecualikan" {{ old('category', $dipDocument->category) == 'dikecualikan' ? 'selected' : '' }}>Informasi Dikecualikan</option>
            </select>
        </div>

        <div>
            <label class="block text-slate-700 mb-1.5 font-semibold">Update File Dokumen (Opsional)</label>
            ...
            @if($dipDocument->file_path)
                <p class="mt-2 text-xs text-slate-500">File saat ini: Tersedia ({{ $dipDocument->file_size }})</p>
            @endif
        </div>
