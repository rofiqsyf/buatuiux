...
@section('title', 'Tambah Berita & SEO Editor')
...
<div class="grid grid-cols-1 lg:grid-cols-3 gap-8 max-w-7xl mx-auto">
    ...
    <div class="lg:col-span-2 space-y-6">
        <div class="bg-white border border-slate-200 rounded-2xl p-6 sm:p-8 shadow-sm">
            ...
            <form id="newsForm" action="{{ route('admin.news.store') }}" method="POST" enctype="multipart/form-data" class="space-y-6 text-sm">
                @csrf
                
                <div>
                    <label class="block text-slate-700 mb-1.5 font-semibold">Judul Berita (SEO Title) *</label>
                    <input type="text" id="titleInput" name="title" value="{{ old('title') }}" required placeholder="Ketik judul artikel berita resmi..." class="w-full bg-slate-50 border border-slate-200 rounded-xl px-3.5 py-2.5 text-slate-800 focus:outline-none focus:border-teal-500 font-medium">
                </div>
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-5">
                    <div>
                        ...
                        <select name="category" required class="w-full bg-slate-50 border border-slate-200 rounded-xl px-3.5 py-2.5 text-slate-800 focus:outline-none focus:border-teal-500">
                            ...
                            <option value="Pengumuman Resmi" {{ old('category') == 'Pengumuman Resmi' ? 'selected' : '' }}>Pengumuman Resmi</option>
                            <option value="Berita Utama" {{ old('category') == 'Berita Utama' ? 'selected' : '' }}>Berita Utama</option>
                        </select>
                    </div>
                    <div>
                        <label class="block text-slate-700 mb-1.5 font-semibold">Tanggal Publikasi *</label>
                        <input type="date" name="published_at" value="{{ old('published_at', date('Y-m-d')) }}" required class="w-full bg-slate-50 border border-slate-200 rounded-xl px-3.5 py-2.5 text-slate-800 focus:outline-none focus:border-teal-500">
                    </div>
                </div>
                ...
                <div>
                    ...
                    <textarea id="summaryInput" name="summary" rows="2" required placeholder="Tuliskan ringkasan 1-2 kalimat (akan muncul pada cuplikan pencarian Google)..." class="w-full bg-slate-50 border border-slate-200 rounded-xl px-3.5 py-2.5 text-slate-800 focus:outline-none focus:border-teal-500">{{ old('summary') }}</textarea>
                </div>
        ...
        <div class="bg-white border border-slate-200 rounded-2xl p-6 shadow-sm space-y-4 sticky top-[130px]">
            ...
            <div class="bg-slate-50 border border-slate-200 p-4 rounded-xl space-y-1.5 font-sans">
                ...
                <p id="seoDescPreview" class="text-xs text-slate-600 leading-relaxed line-clamp-3">
                    Ringkasan deskripsi artikel berita akan tampil di sini pada hasil pencarian mesin pencari Google...
                </p>
            </div>
